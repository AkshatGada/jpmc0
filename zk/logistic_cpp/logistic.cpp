...
void Example_2_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void Example_2_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_main_input_signal_no() {return 57;}

uint get_total_signal_no() {return 348;}

uint get_number_of_components() {return 3;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 339;}
...
ctx->componentMemory[coffset].signalStart = soffset;
ctx->componentMemory[coffset].inputCounter = 57;
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
ctx->componentMemory[coffset].subcomponents = new uint[1]{0};
}
...
Fr_lt(&expaux[0],&lvar[0],&circuitConstants[5]); // line circom 21
while(Fr_isTrue(&expaux[0])){
{
PFrElement aux_dest = &signalValues[mySignalStart + ((1 * (Fr_toInt(&lvar[0]) + 1)) + 60)];
// load src
Fr_mul(&expaux[2],&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[0])) + 3)],&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[0])) + 31)]); // line circom 22
Fr_add(&expaux[0],&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[0])) + 60)],&expaux[2]); // line circom 22
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
Fr_neq(&expaux[0],&signalValues[mySignalStart + 1],&circuitConstants[1]); // line circom 36
if(Fr_isTrue(&expaux[0])){
{
PFrElement aux_dest = &signalValues[mySignalStart + 89];
// load src
Fr_div(&expaux[0],&circuitConstants[2],&signalValues[mySignalStart + 1]); // line circom 36
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
}else{
...
// load src
Fr_neg(&expaux[2],&signalValues[mySignalStart + 1]); // line circom 38
Fr_mul(&expaux[1],&expaux[2],&signalValues[mySignalStart + 89]); // line circom 38
Fr_add(&expaux[0],&expaux[1],&circuitConstants[2]); // line circom 38
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
...
assert(Fr_isTrue(&expaux[0]));
Fr_eq(&expaux[0],&signalValues[mySignalStart + 59],&signalValues[mySignalStart + 2]); // line circom 42
...
Example_2_create(1,0,ctx,"main",0);
Example_2_run(0,ctx);
}
